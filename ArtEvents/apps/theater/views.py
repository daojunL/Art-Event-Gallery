...
def TheaterPage(request):
    ...
    # print((end - start))
    return render(request, 'SearchTheaterPage.html', context={'content': content})

def TheaterQuery(request):
    # post method
    timeDict = {'1': 7,
                '2': 30,
                '3': 90,
    ...
    if city != '':
        ...
        if len(lid) == 0:
        #     pass
            return render(request, 'SearchTheaterPage.html', {'error_message':'Events not found'})
        if len(lid) == 1:
